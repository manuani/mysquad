...
describe('scoreExpert', () => {
  ...
  it('returns > 0 when topic overlaps domain slug', () => {
    const expert = makeExpert({
      domainTags: [{ id: 'tag-1', expertId: 'exp-1', domain: 'saas_pricing', confidence: 1.0, verified: true, createdAt: '' }],
    });
    expect(scoreExpert(expert, new Set(['saas', 'pricing']))).toBeGreaterThan(0);
  });

  it('higher confidence amplifies the score', () => {
    const make = (confidence: number) => makeExpert({
      domainTags: [{ id: 't', expertId: 'exp-1', domain: 'fundraising', confidence, verified: false, createdAt: '' }],
    });
  ...
  it('returns 0 when no topic tokens overlap any tag', () => {
    const expert = makeExpert({
      domainTags: [{ id: 't', expertId: 'exp-1', domain: 'legal_compliance', confidence: 1.0, verified: false, createdAt: '' }],
    });
...
describe('matchedDomains', () => {
  it('returns only domains with token overlap', () => {
    const expert = makeExpert({
      domainTags: [
        { id: '1', expertId: 'exp-1', domain: 'saas_pricing', confidence: 0.9, verified: false, createdAt: '' },
        { id: '2', expertId: 'exp-1', domain: 'legal_compliance', confidence: 0.8, verified: false, createdAt: '' },
      ],
    });
...
describe('matchExperts', () => {
  ...
  function buildFakeClient(experts: ExpertWithTags[]) {
    return {
      async query(_sql: string, params: unknown[]) {
        ...
        if (Array.isArray(params[0])) {
          ...
          return {
            rows: experts.flatMap((e) =>
              e.domainTags.map((t) => ({
                id: t.id, expert_id: t.expertId, domain: t.domain,
                confidence: t.confidence, verified: t.verified, created_at: new Date(),
              })),
            ),
          };
        }
        return {
          rows: experts.map((e) => ({
            id: e.id, tenant_id: e.tenantId, name: e.name, email: e.email,
            bio: null, linkedin_url: null, status: e.status,
  ...
  it('returns matches sorted by relevance score descending', async () => {
    const experts: ExpertWithTags[] = [
      makeExpert({ id: 'low',  name: 'Bob',   domainTags: [makeTag('t1', 'low',  'legal_compliance')] }),
      makeExpert({ id: 'high', name: 'Alice', domainTags: [makeTag('t2', 'high', 'saas_pricing')] }),
    ];
    const matches = await matchExperts(TC, buildFakeClient(experts) as never, 'saas pricing strategy', 5);
  ...
  it('excludes experts with relevance score of 0', async () => {
    const experts: ExpertWithTags[] = [
      makeExpert({ id: 'no-match', name: 'Dave', domainTags: [makeTag('t', 'no-match', 'legal_compliance')] }),
    ];
    const matches = await matchExperts(TC, buildFakeClient(experts) as never, 'financial modeling', 5);
    expect(matches).toHaveLength(0);
  });
});
