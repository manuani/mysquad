...
ROWS = [
    ...
    ("P3", "S12 — Admin Console", "admin-console-api", "User management: invite, role assignment, deactivation", "Done", "Engineering", "sprint25: GET/POST/PATCH/DELETE /tenants/:id/users endpoints"),
    ...
    ("P3", "S12 — Admin Console", "Admin UI", "Internal admin web app (separate from founder-facing demo UI)", "Done", "Product", "sprint26: vanilla HTML/JS SPA served at /admin; login, tenant list, user management"),
    ...
    ("P4", "S16 — Multi-provider LLM Routing", "routing", "OpenAI provider implementation", "Done", "Engineering", "sprint27: OpenAIProvider; gpt-4o (high) + gpt-4o-mini (good)"),
    ("P4", "S16 — Multi-provider LLM Routing", "routing", "Bedrock provider implementation", "Done", "Engineering", "sprint27: BedrockProvider; Llama 3 via ConverseCommand; opensource tier"),
    ("P4", "S16 — Multi-provider LLM Routing", "routing", "Failover: if primary provider errors, retry on next tier", "Done", "Engineering", "sprint27: RoutingService cascades tiers on error; 17 tests"),
    ("P4", "S16 — Multi-provider LLM Routing", "routing", "Cost-per-call tracking per provider per tenant", "Done", "Engineering", "sprint27: computeCostMicro; TIER_COST_MICRO_PER_1K; totalCostMicro in result + usage callback"),

    ("P4", "S17 — Background Workers", "worker", "worker app: BullMQ job queue over Redis (apps/worker stub)", "Done", "Engineering", "sprint28: apps/worker; BullMQ v5 over ioredis; graceful SIGTERM shutdown"),
    ("P4", "S17 — Background Workers", "worker", "Async brain indexing (don't block meeting response)", "Done", "Engineering", "sprint28: QUEUE_BRAIN_INDEX → processBrainIndex → createBrainContentItem"),
    ("P4", "S17 — Background Workers", "worker", "Async Neo4j graph updates", "Done", "Engineering", "sprint28: QUEUE_NEO4J_GRAPH → processNeo4jGraph → MERGE edges; idempotent"),
    ("P4", "S17 — Background Workers", "worker", "Dead-letter queue + retry with exponential backoff", "Done", "Engineering", "sprint28: 5 retries, exp backoff 1s→30s max; DLQ for exhausted jobs; 9 tests"),
